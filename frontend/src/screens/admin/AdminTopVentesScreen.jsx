    ...
    <div className="tv-wrap">
      ...
      <div className="tv-kpis">
        <div className="tv-kpi">
          ...
          <div className="val">{fmtFranc(data?.totaux?.caAnnee)}</div>
        </div>
        <div className="tv-kpi">
          <div className="lbl">Ventes totales (qté)</div>
          <div className="val">{fmtQty(data?.totaux?.venteAnnee)}</div>
        </div>
        <div className="tv-kpi">
          <div className="lbl">{groupBy === "rayon" ? "Rayons" : "Fournisseurs"}</div>
          <div className="val">{fmtQty(data?.nbGroupes)}</div>
        </div>
      </div>
            ...
            {isLoading ? (
              ...
              lignes.map((l, i) => (
                ...
                >
                  ...
                  <td className="num">{fmtQty(l.nbArticles)}</td>
                  <td className="num">{fmtQty(l.venteAnnee)}</td>
                  <td className="num">
...
const DetailModal = ({ dossier, drill, onClose }) => {
  ...
  return (
    <Modal onClose={onClose} overlayClassName="tv-overlay" contentClassName="tv-modal">
      <h3>
