...
// sans duplication de logique.
//
// Arborescence cible sur RCOMMUN (le dossier du TRIGRAMME regroupe controle_cmd
// ET prepa_cmd de l'entreprise, pour plus de lisibilité) :
//   <base collecteur>/<TRIGRAMME>/controle_cmd/<NUMCDE>_<AAAA-MM-JJ>_<fournisseur>/
//       <NUMCDE>_<AAAA-MM-JJ>_<fournisseur>.pdf         (rapport)
//       signalement_<REF>_<type>.<ext>                  (photos problèmes)
//
// IMPORTANT — Base « collecteur » SANS migration de base de données :
 ...
 *
 * Le dossier <TRIGRAMME> est placé EN PREMIER (il regroupe controle_cmd et
 * prepa_cmd de l'entreprise). La base « collecteur » est DÉRIVÉE du chemin de
 * dépôt existant de l'entreprise (voir deriveCollecteurBase) -> aucune migration
 * de base nécessaire.
 *
 * @param {object} entreprise  Document Entreprise (getter cheminRapportReception + trigramme)
 * @param {object} reception   Document Reception (numcde + commandeInfo)
...
export const buildControleCmdDir = (entreprise, reception) => {
  ...
  const commandeDir = buildBaseName(reception);
  return path.join(base, trigramme, CONTROLE_CMD_DIR, commandeDir);
};
